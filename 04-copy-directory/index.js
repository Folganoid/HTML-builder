...
const dir = path.join(__dirname, path.sep, 'files');
const dirCopy = path.join(__dirname, path.sep, 'files-copy');

fs.rm(dirCopy, { recursive: true, force: true }, () => {
  mkdir(dirCopy, { recursive: true }).then(() => {
    readdir(dir).then((res) => {
      for (let file of res) {
        let fullName = path.join(dir, path.sep, file);
        let fullNameCopy = path.join(dirCopy, path.sep, file);
